...
tr_all=tr_all_440(tr_dat,train1_data);
...
H_train2= H_train_440( H_train, H_train1); 

H_test= lcksvd_buildH(Test_label);%目标域用于测试的标签矩阵
%
for dictsize=440% 字典数=训练数
sumd=sumd+1;
% clear Dinit%字典初始化%
...
for alpha1=1%1:6 %
    for beta1=1%1:6%
         for gam1=4%1:6%
             for gams1=6%1:6%86.91
          alpha=PA(alpha1);
          beta=PA(beta1);
          gam=PA(gam1);
          gams=PA(gams1);
   
Train_label=  Train_label_440( Train1_label,train1_data_label); 
...
[D,X,obj] = gaikernel_Learn_D_X(tr_all,Dinit,Xinit,alpha,beta,gam,gams,max_iter,U,Y_range); %用训练样本学习得到字典和编码系数的函数 
% [D,X,obj] = gaiLearn_D_X(tr_all,Dinit,Xinit,alpha,beta,gam,gams,max_iter,U,Y_range);
% P=X';
